...
def updatePhi(i, j, k, my_object, phi, g, x1, x2, x3, obstcle_map):
    ...
    ## S
    with hcl.if_(obstcle_map[i,j]!=0):
        phi[i, j, k] = 1000000
    with hcl.else_():
...
def TTR_3D(my_object, g):
    def solve_phiNew(phi, x1, x2, x3, obstcle_map):
                ...
                with hcl.for_(l_i, h_i, name="i") as i:
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            updatePhi(i, j, k, my_object, phi, g, x1, x2, x3, obstcle_map)
                            # debug2[0] = j
                EvalBoundary(phi, g)
                ...
                with hcl.for_(l_i, h_i, name="i") as i:
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            ...
                            j2 = phi.shape[1] - j - 1
                            k2 = phi.shape[2] - k - 1
                            updatePhi(i2, j2, k2, my_object, phi, g, x1, x2, x3, obstcle_map)
                EvalBoundary(phi, g)
                ...
                with hcl.for_(l_i, h_i, name="i") as i:
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            j2 = phi.shape[1] - j - 1
                            k2 = phi.shape[2] - k - 1
                            updatePhi(i, j2, k2, my_object, phi, g, x1, x2, x3, obstcle_map)
                EvalBoundary(phi, g)
                ...
                with hcl.for_(l_i, h_i, name="i") as i:
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            i2 = phi.shape[0] - i - 1
                            j2 = phi.shape[1] - j - 1
                            updatePhi(i2, j2, k, my_object, phi, g, x1, x2, x3, obstcle_map)
                EvalBoundary(phi, g)
                ...
                with hcl.for_(l_i, h_i, name="i") as i:
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            i2 = phi.shape[0] - i - 1
                            k2 = phi.shape[2] - k - 1
                            updatePhi(i2, j, k2, my_object, phi, g, x1, x2, x3, obstcle_map)
                EvalBoundary(phi, g)
            ...
            with hcl.Stage("Sweep_6"):
                with hcl.for_(l_i, h_i, name="i") as i:
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            i2 = phi.shape[0] - i - 1
                            updatePhi(i2, j, k, my_object, phi, g, x1, x2, x3, obstcle_map)
                EvalBoundary(phi, g)
            ...
            with hcl.Stage("Sweep_7"):
                with hcl.for_(l_i, h_i, name="i") as i:
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            j2 = phi.shape[1] - j - 1
                            updatePhi(i, j2, k, my_object, phi, g, x1, x2, x3, obstcle_map)
                EvalBoundary(phi, g)
            ...
            with hcl.Stage("Sweep_8"):
                with hcl.for_(l_i, h_i, name="i") as i:
                    with hcl.for_(l_j, h_j, name="j") as j:
                        with hcl.for_(l_k, h_k, name="k") as k:
                            k2 = phi.shape[2] - k - 1
                            updatePhi(i, j, k2, my_object, phi, g, x1, x2, x3, obstcle_map)
                EvalBoundary(phi, g)
    ...
    hcl_obstcle_map = hcl.placeholder((g.pts_each_dim[0],g.pts_each_dim[1]), name="obstcle_map", dtype=hcl.Float())

    # Create a static schedule -- graph
    s = hcl.create_schedule([phi, x1, x2, x3, hcl_obstcle_map], solve_phiNew)
